# 클래스
# 클래스는 객체 지향 프로그래밍을 지원하는 중요한 개념
# 추상화된 데이터와 함수를 하나의 단위로 묶어 클래스를 만들 수 있음
# 클래스를 활용해 인스턴스 단위를 생성하여 객체 단위로 사용이 가능함
...
class Person3:
    ...
    def information(self):
        return "i'm fron %s and i use %s" % (self.national, self.language)

    def favorite(self, color):
        return "i love %s" % color

# 생성자 : __init__ : 객체가 생성될 때 자동으로 호출 됨. 매개변수를 전달 받아 인스턴스 속성을 초기화 할 수 있다.
class Student:
    # 클래스 변수 : 모든 인스턴스가 공유
    total_students = 0
...
# 1. 지역 네임 스페이스 : 현재 함수나 메소드 내의 네임 스페이스
# 2. 인스턴스 네임 스페이스 : 인스턴스 객체의 네임 스페이스
# 3. 클래스 네임 스페이스 : 클래스 객체의 네임 스페이스
# 4. 전역 네임 스페이스 : 모듈 내의 전역 스페이스
# 5. 내장 네임 스페이스 : 파이썬 내장 함수와 예외를 포함하는 네임 스페이스
...
def outer_function():
    ...
    print('지역(외부 함수) : %s' % variable)

    def inner_function():
        variable = 'inner variable'
        print('지역(내부 함수) : %s' % variable)
...
class Test_class:
    ...
    def class_function(self):
        ...
        print('클래스 지역 : %s' % variable)

obj = Test_class('instance varibale')
...
class BankAccount:
    ...
    def withdraw(self, amount):
        if 0 < amount <= self.__balance:
            self.__balance -= amount
        else: raise ValueError('잔액이 부족하거나 잘 못 된 출금 금액입니다.')

# 계좌 생성
my_account = BankAccount('123-456-789', 100000)

# 계좌 번호 조회
print('계좌 번호 : %s' % my_account.account_number)
...
# 비공개 속성에 접근하는 메소드 사용
print('잔액 : %d' % my_account.get_balance())

# 입금
my_account.deposit(50000)
print('입금 후 잔액 : %d' % my_account.get_balance())

# 출금
my_account.withdraw(30000)
print('출금 후 잔액 : %d' % my_account.get_balance())
...
class Student(Person):
    pass # 상속만 받고 내용은 추가하지 않음 (구문상 필요는 하지만 프로그램이 아무 작업도 하지 않기를 원하는 경우 사용. 없어도 되긴 함)
...
class Learner1(Person1):
    ...
    def learn(self):
        return 'i am learing %s' % self.subject
